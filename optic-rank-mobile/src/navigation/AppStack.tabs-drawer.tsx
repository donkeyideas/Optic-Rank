/**
 * Option B: Bottom Tabs + Drawer Hybrid
 * 4 key bottom tabs (Home, Keywords, AI, Profile)
 * Hamburger icon on each screen opens drawer for all other pages
 */
  ...
  return (
    <Tab.Navigator
      screenOptions={({ route }) => ({
        headerShown: false,
        tabBarIcon: ({ color }) => (
          <Feather name={TAB_ICONS[route.name as keyof TabParamList]} size={22} color={color} />
        ),
        tabBarActiveTintColor: "#c0392b",
        tabBarInactiveTintColor: "#999999",
        tabBarLabelStyle: {
          fontFamily: Platform.select({ ios: "IBMPlexSans-Medium", android: "IBMPlexSans-Medium", default: undefined }),
          fontSize: 9,
        },
        tabBarStyle: {
    ...
    >
      ...
      <Tab.Screen name="KeywordsTab" component={KeywordsScreen} options={{ tabBarLabel: "Keywords" }} />
      <Tab.Screen name="AITab" component={AIInsightsScreen} options={{ tabBarLabel: "AI" }} />
...
const DRAWER_SECTIONS: DrawerSection[] = [
  {
    title: "",
    items: [{ screen: "HomeTabs", label: "Dashboard", icon: "home" }],
  },
  {
    title: "SEO TOOLS",
    items: [
...
function CustomDrawerContent(props: DrawerContentComponentProps) {
  ...
  return (
    <View style={[ds.container, { backgroundColor: colors.surface }]}>
      <View style={[ds.header, { paddingTop: insets.top + 16, borderBottomColor: colors.border }]}>
        <Text style={[ds.brand, { color: colors.ink }]}>Optic Rank</Text>
        <Text style={[ds.subtitle, { color: colors.inkMuted }]}>
          {profile?.full_name ?? "SEO Intelligence"}
        </Text>
      </View>
      <ScrollView style={ds.scroll} showsVerticalScrollIndicator={false}>
        {DRAWER_SECTIONS.map((section) => (
          <View key={section.title || "home"} style={ds.section}>
            {section.title !== "" && (
              <Text style={[ds.sectionTitle, { color: colors.inkMuted }]}>{section.title}</Text>
            )}
            {section.items.map((item) => {
              ...
              return (
                <TouchableOpacity
                  key={item.screen}
                  style={[ds.item, isActive && { backgroundColor: isDark ? "rgba(255,255,255,0.08)" : "rgba(0,0,0,0.05)" }]}
                  onPress={() => props.navigation.navigate(item.screen)}
                  activeOpacity={0.7}
                >
                  <Feather name={item.icon} size={16} color={isActive ? "#c0392b" : colors.inkSecondary} />
                  <Text style={[ds.itemLabel, { color: isActive ? "#c0392b" : colors.ink }, isActive && ds.itemLabelActive]}>
                    {item.label}
                  </Text>
                </TouchableOpacity>
              );
        ...
        <TouchableOpacity style={ds.signOut} onPress={() => signOut().catch(() => Alert.alert("Error", "Failed to sign out."))} activeOpacity={0.7}>
          <Feather name="log-out" size={16} color={colors.inkMuted} />
          <Text style={[ds.signOutLabel, { color: colors.inkMuted }]}>Sign Out</Text>
        </TouchableOpacity>
      </ScrollView>
    </View>
  );
...
const ds = StyleSheet.create({
  ...
  brand: { fontFamily: fonts.serifBlack, fontSize: 18, fontWeight: "900" },
  subtitle: { fontFamily: fonts.sans, fontSize: 11, marginTop: 2 },
  scroll: { flex: 1 },
  section: { paddingTop: 12, paddingHorizontal: 12 },
  sectionTitle: { fontFamily: fonts.sansBold, fontSize: 9, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4, marginLeft: 8 },
  item: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 10, paddingHorizontal: 8 },
  itemLabel: { fontFamily: fonts.sans, fontSize: 13 },
  itemLabelActive: { fontFamily: fonts.sansBold },
  signOut: { flexDirection: "row", alignItems: "center", gap: 12, paddingVertical: 10, paddingHorizontal: 20, marginTop: 8, marginBottom: 32 },
  signOutLabel: { fontFamily: fonts.sans, fontSize: 13 },
});

// ── Drawer wrapping everything ───────────────────────────────────────────────
...
export default function AppStack() {
  ...
  return (
    ...
    >
      {/* Tab hub */}
      <Drawer.Screen name="HomeTabs" component={TabNavigator} options={{ headerShown: false, title: "Home" }} />
