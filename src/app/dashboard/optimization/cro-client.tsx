...
export function CroClient({
  ...
  // Split keywords into top performers (pos 1-5) and opportunities (pos 6+)
  const topPerformers = keywordsWithRevenue.filter(
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2">
        <DollarSign size={18} className="text-editorial-red" />
      ...
      <div className="border border-rule bg-surface-card">
        ...
        {showForm && (
          <form
            ...
            className="border-b border-rule bg-surface-raised/50 px-5 py-4"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div>
                <label className="mb-1 block text-[10px] font-bold uppercase tracking-[0.12em] text-ink-muted">
                  Name
                </label>
                <input
                  name="name"
        ...
        {conversionGoals.length > 0 ? (
            ...
            {conversionGoals.map((goal) => (
              <div key={goal.id} className="flex items-center justify-between px-5 py-3">
                <div>
                  ...
                  <span className="ml-2 text-[10px] text-ink-muted capitalize">
                    {goal.goal_type.replace("_", " ")}
                  </span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-[11px] text-ink-muted">
                    ${Number(goal.estimated_value).toFixed(2)} / conv
                  </span>
                  <span className="text-[11px] text-ink-muted">
                    {(Number(goal.estimated_conversion_rate) * 100).toFixed(1)}% rate
                  </span>
                  <button
                    type="button"
          ...
          <div className="px-6 py-8 text-center">
            ...
            <p className="text-[12px] text-ink-muted">
              No conversion goals set. Add goals to see revenue estimates.
            </p>
          </div>
        )}
      </div>
        ...
        <div className="border border-rule bg-surface-card">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              ...
              <tbody>
                {topPerformers.slice(0, 15).map((kw) => (
                  <tr key={kw.keywordId} className="border-b border-rule last:border-0">
                    ...
                    <td className="px-4 py-2.5 text-right text-[11px] text-ink-muted hidden md:table-cell">
                      {kw.estimatedCtr}%
                    </td>
                    <td className="px-4 py-2.5 text-right text-[11px] text-ink-muted hidden md:table-cell">
                      {kw.estimatedTraffic.toLocaleString()}
                    </td>
      ...
      {opportunities.length > 0 && (
        <div className="border border-rule bg-surface-card">
          <div className="border-b border-rule px-5 py-3 flex items-center justify-between">
            ...
            <span className="text-[10px] font-semibold text-editorial-red">
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {opportunities.slice(0, 15).map((kw) => {
                  // Estimate potential if moved to position 3
                  const potentialCtr = 0.11;
                  ...
                  const avgValue = conversionGoals.length > 0
                    ...
                    : 10;
                  const potentialRev = kw.searchVolume * potentialCtr * avgRate * avgValue;
